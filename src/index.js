#!/usr/bin/env node
/**
 * mcp-unhrdb — Model Context Protocol server for the UN Human Rights Database.
 *
 * Thin stdio wrapper over the existing UNHRDB HTTP API (FTS5 search over
 * 203K paragraphs of Treaty Body General Comments, jurisprudence and
 * Special Procedures reports). Exposes two tools:
 *
 *   - search_paragraphs   full-text search, scope/committee/year filters
 *   - lookup_by_citation  resolve a UN signature + ¶ number to its paragraph
 *
 * Config (env):
 *   UNHRDB_API_BASE     default https://150.254.115.204/unhrdb-api/api
 *   UNHRDB_API_KEY      optional bearer token sent as `X-API-Key`. Set this
 *                        when pointing at the hardened /unhrdb-mcp/ route so
 *                        MCP traffic is rate-limited & revocable separately
 *                        from the public dashboard route.
 *   UNHRDB_INSECURE_TLS  "1" (default) accepts the VM's self-signed cert.
 *                        Set "0" once the API is behind a trusted cert.
 */
...
// The VM serves the API behind a self-signed cert. Scope the relaxed TLS
// to an explicit agent rather than the global NODE_TLS_REJECT_UNAUTHORIZED
// so we don't weaken TLS for any other request this process might make.
...
const server = new McpServer({ name: 'mcp-unhrdb', version: '0.1.0' });
...
server.registerTool(
  ...
  async ({ citation }) => {
    ...
    // Match on the printed paragraph number `n`, falling back to idx.
...
console.error(`mcp-unhrdb ready · API ${API_BASE} · key ${API_KEY ? 'set' : 'none'} · TLS ${INSECURE_TLS ? 'insecure (self-signed OK)' : 'verified'}`);
